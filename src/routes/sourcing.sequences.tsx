...
import { listSequences, upsertSequence, deleteSequence } from "@/server/sourcing.functions";
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_1fr]">
      <section className="space-y-3">
        ...
        {items.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            No sequences yet. Create one to send personalized outreach.
          </p>
        ) : (
          items.map((s) => (
            <div
              key={s.id}
              className="flex items-center gap-3 rounded-2xl border border-white/10 bg-card/40 p-4"
            >
      ...
      <section>
        {editing ? (
          <div className="space-y-3 rounded-2xl border border-white/10 bg-card/40 p-5">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Use {"{{name}}"}, {"{{role}}"}, {"{{sender}}"}. AI will personalize one specific
                line per send.
              </p>
            </div>
            <div>
          ...
          <p className="text-sm text-muted-foreground">
            Pick a sequence to edit, or create a new one.
          </p>
        )}
      </section>
    </div>
