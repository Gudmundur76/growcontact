...
function PushCandidateGreenhouseBtn({ candidateId }: { candidateId: string }) {
  const push = useServerFn(pushCandidateToGreenhouse);
  const [busy, setBusy] = useState(false);
  async function onClick() {
    setBusy(true);
    try {
      const r = await push({ data: { candidateId } });
      toast.success(r.externalId ? `Pushed to Greenhouse (${r.externalId})` : "Pushed to Greenhouse");
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Failed to push");
    } finally {
      setBusy(false);
  ...
  return (
    <Button size="sm" variant="outline" onClick={onClick} disabled={busy} className="gap-1.5">
      <Leaf className="h-3.5 w-3.5" /> {busy ? "Pushing…" : "Greenhouse"}
    </Button>
  );
}
